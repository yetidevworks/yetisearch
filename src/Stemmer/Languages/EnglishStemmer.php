...
class EnglishStemmer extends BaseStemmer
{
    ...
    private function step1a(): void
    {
        ...
        if ($this->replaceSuffix('sses', 'ss')) {
            return;
        }
        if ($this->replaceSuffix('ies', 'i')) {
            return;
        }
        if ($this->endsWith('ss')) {
            return;
        ...
        if ($this->endsWith('us')) {
            return; // Don't remove s from words like 'us'
        }
        if ($this->endsWith('is')) {
            return; // Don't remove s from words like 'this', 'is'
        }
        if ($this->removeSuffix('s')) {
            return;
        }
    }
    ...
    private function step1b(): void
    {
        ...
        if ($modified) {
            // Handle at, bl, iz
            if ($this->replaceSuffix('at', 'ate')) {
                return;
            }
            if ($this->replaceSuffix('bl', 'ble')) {
                return;
            }
            if ($this->replaceSuffix('iz', 'ize')) {
                return;
            }
    ...
    private function step4(): void
    {
        ...
        if ($this->endsWith('ion')) {
            ...
            if (
                $this->measureGreaterThan($base, 1) &&
