...
- **Pages, not chunks, with `unique_by_route`**: When results are deduplicated by route, hybrid search ranks each page by its best chunk on each side before fusion. Without this, the many chunks of one long page filled the vector ranking and their scores were summed by the deduplication, so a page won by length instead of by how well it matched.
